...
public class ActivityIndicator: UIView {
    ...
    class var shared: ActivityIndicator {
        struct Singleton {
            static let instance = ActivityIndicator(frame: CGRect(origin: CGPoint(x: 0,y: 0),size: CGSize(width: Config().size,height: Config().size)))
        }
    ...
    public class func show(title: String?, animated : Bool) {
        ...
        let loader = ActivityIndicator.shared
    ...
    public class func hide() {
        NotificationCenter.default.removeObserver(ActivityIndicator.shared)
